		...
		<div class="flex flex-1 flex-col items-start gap-2">
			<div class="flex items-center gap-4">
				...
				<p
					...
					]} bg-surface text-[0.65rem] text-{languageColor[
						project.primaryLanguage
					]}"
				>
					{project.primaryLanguage}
				...
				<p
					class="px-1.5 py-px border rounded border-{statusColor[
						project.status
					]} bg-surface text-[0.65rem] text-{statusColor[project.status]}"
				>
					{project.status}
				</p>
			</div>
			<p class="text-overlay0 text-xs text-left">{project.description}</p>
			<div class="flex items-center gap-2 flex-wrap">
				{#each project.technologies.slice(0, 3) as technology}
					<span class="text-[0.65rem] text-mauve">@{technology}</span>
				{/each}
			</div>
		</div>
	</div>
